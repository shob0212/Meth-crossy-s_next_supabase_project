...
import { Trip, ItineraryEvent, Category, Language } from "@/lib/types";
...
export function CalendarView({ trip, language = "ja" }: CalendarViewProps) {
  ...
  const eventsOnSelectedDate = trip.events
    ...
    <div className="pb-24">
      <div className="bg-white dark:bg-gray-800 rounded-[32px] p-6 shadow-sm border border-gray-100 dark:border-gray-700 mb-6">
        <div className="flex justify-between items-center mb-6">
          <button
            onClick={prevMonth}
            className="w-8 h-8 rounded-full bg-gray-50 dark:bg-gray-700 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-600"
          >
        ...
        <div className="grid grid-cols-7 mb-4">
          {weekdays.map((day, i) => (
            <div
              key={i}
              className="text-center text-xs font-bold text-gray-300 dark:text-gray-600"
            >
              {day}
            </div>
          ))}
              ...
              <div key={day} className="flex flex-col items-center">
                ...
                >
                  ...
                  {hasEvents && !isSelected && (
                    <div className="absolute bottom-1 w-1 h-1 rounded-full bg-mint-400"></div>
                  )}
                </button>
              </div>
            );
        ...
        {eventsOnSelectedDate.length === 0 ? (
          <div className="text-center py-8 bg-white dark:bg-gray-800 rounded-[24px] border border-gray-100 dark:border-gray-700">
            <p className="text-gray-400 text-sm">
              {language === "ja" ? "予定がありません" : "No events"}
            </p>
          </div>
        ) : (
          eventsOnSelectedDate.map((event) => (
            <div
              key={event.id}
              className="bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-sm border border-gray-100 dark:border-gray-700 flex gap-4"
            >
