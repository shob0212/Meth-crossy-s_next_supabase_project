...
const getCategoryStyles = (cat: Category) => {
  switch (cat) {
    case Category.Food:
      return { bg: "bg-peach-50 dark:bg-peach-900/20", text: "text-peach-400" };
    case Category.Transport:
      return { bg: "bg-blue-50 dark:bg-blue-900/20", text: "text-blue-400" };
    case Category.Activity:
      return { bg: "bg-mint-50 dark:bg-mint-900/20", text: "text-mint-400" };
...
export function Timeline({
  ...
  const handleSaveEvent = () => {
    if (!newEventData.title || !newEventData.startTime || !newEventData.date)
      return;
    const event = {
    ...
    editingEventId
      ? onUpdateEvent(event as ItineraryEvent)
      : onAddEvent(event as ItineraryEvent);
    ...
    <div className="pb-24 select-none">
      {sortedDates.map((dateStr) => {
        const dayEvents = events
          .filter((e) => (e.date || startDateStr) === dateStr)
          ...
          <div key={dateStr} className="mb-10 last:mb-0">
            <div className="flex justify-between items-center mb-6 py-2 sticky top-0 bg-ivory/95 dark:bg-gray-900/95 backdrop-blur-sm z-20 border-b border-gray-100/50 dark:border-gray-800/50">
              ...
              {!isReadOnly && (
                <button
                  onClick={() => {
                    setActiveDateForAi(dateStr);
                    setIsAiModalOpen(true);
                  }}
                  className="text-mint-500 bg-white dark:bg-gray-800 px-4 py-1.5 rounded-full text-xs font-bold border border-mint-200 dark:border-mint-900 hover:bg-mint-50 dark:hover:bg-mint-900/20 transition-colors flex items-center gap-1 shadow-sm"
                >
            ...
            <div className="relative pl-4 border-l border-dashed border-gray-300 dark:border-gray-700 ml-3.5 space-y-2">
              ...
              {!isReadOnly && dayEvents.length > 0 && (
                <InsertZone
                  time={
                    dayEvents.length > 0
                      ? addTime(dayEvents[0].startTime, -60)
                      : "09:00"
                  }
                  onAdd={(t) => handleOpenAdd(t, dateStr)}
                />
              )}
              ...
              {dayEvents.map((event, index) => {
                ...
                const insertTime = nextEvent
                  ? getMidTime(event.startTime, nextEvent.startTime)
                  : addTime(event.startTime, 60);
                  ...
                  <React.Fragment key={event.id}>
                    <div className="relative flex gap-4 group ml-6">
                      ...
                      <div className="flex-1 bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-sm border border-gray-200 dark:border-gray-700 hover:border-mint-200 dark:hover:border-mint-900 hover:shadow-md transition-all relative group/card z-20">
                        <div
                          className={`absolute top-0 right-0 px-4 py-2 rounded-bl-2xl rounded-tr-[24px] text-[10px] font-bold tracking-wider uppercase ${style.bg} ${style.text}`}
                        >
                          {t.categories[event.category] || event.category}
                        </div>
                        <div className="flex items-start gap-3">
                          <div>
                            <p className="text-2xl font-bold text-gray-800 dark:text-white mb-1 tracking-tight">
                              {event.startTime}
                            </p>
                            <h3 className="font-semibold text-lg text-gray-700 dark:text-gray-200 leading-snug">
                              {event.title}
                            </h3>
                            ...
                            {event.notes && (
                              <p className="text-gray-400 text-xs mt-3 bg-gray-50 dark:bg-gray-700 p-2 rounded-xl border border-gray-100 dark:border-gray-600 inline-block">
                                {event.notes}
                              </p>
                            )}
                          </div>
                        ...
                        {!isReadOnly && (
                          <div className="flex gap-2 mt-4 opacity-0 group-hover/card:opacity-100 transition-opacity absolute bottom-4 right-4 z-30 pointer-events-auto">
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-50 dark:bg-gray-700 text-gray-400 hover:text-mint-500 hover:bg-mint-50 dark:hover:bg-mint-900 transition-colors"
                            >
                              <Pencil size={14} />
                            </button>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteWithConfirm(event.id);
                              }}
                              className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-50 dark:bg-gray-700 text-gray-400 hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900 transition-colors"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
                    ...
                    {!isReadOnly && (
                      <InsertZone
                        time={roundToNearest5(insertTime)}
                        onAdd={(t) => handleOpenAdd(t, dateStr)}
                      />
                    )}
                  </React.Fragment>
          ...
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-[32px] p-8 shadow-xl border border-gray-100 dark:border-gray-700 max-h-[85vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-4">
              <input
                ...
                className="w-full bg-ivory dark:bg-gray-700 dark:text-white rounded-2xl p-4 outline-none text-lg placeholder-gray-300 border border-transparent focus:border-mint-200 transition-colors"
                placeholder={t.title}
                autoFocus
              />
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="flex-1 bg-ivory dark:bg-gray-700 dark:text-white rounded-2xl p-4 outline-none border border-transparent focus:border-mint-200 appearance-none"
                  >
                    {HOURS.map((h) => (
              ...
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  className="w-full bg-ivory dark:bg-gray-700 dark:text-white rounded-2xl p-4 outline-none text-sm border border-transparent focus:border-mint-200 appearance-none"
                >
              ...
              <button
                ...
                className="w-full bg-mint-400 text-white py-4 rounded-full font-bold mt-2 shadow-sm hover:bg-mint-500 transition-colors"
              >
                {t.save}
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-[32px] p-8 shadow-xl border border-gray-100 dark:border-gray-700 text-center">
            ...
            <p className="text-gray-400 text-sm mb-6">
              Let Gemini plan your day in {activeDateForAi}.
            </p>
        ...
        <div className="fixed z-50 bottom-32 right-6">
          <button
            onClick={() =>
              handleOpenAdd(
                "09:00",
                sortedDates[0] || tripDates.split(" - ")[0],
              )
            }
...
const InsertZone = ({
  ...
  <div
    className={`relative h-6 group ml-6 flex items-center cursor-pointer z-10 ${className}`}
    onClick={() => onAdd(time)}
  >
