...
import { Heart, Image, Plus, Trash2 } from "lucide-react";
import type { Language, Memory, User } from "@/lib/types";
			...
			{memories.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-white dark:bg-gray-800 rounded-[32px] border border-gray-200 dark:border-gray-700">
					...
					{!isReadOnly && (
						<button
							...
							className="bg-mint-400 text-white px-6 py-3 rounded-full font-bold shadow-sm hover:bg-mint-500 transition-colors"
						>
							{t.addNote}
						</button>
					)}
				...
				<div className="grid grid-cols-1 gap-4">
					{memories.map((memory, idx) => (
						<div
							key={memory.id}
							className="bg-white dark:bg-gray-800 p-6 rounded-[24px] border border-gray-200 dark:border-gray-700 flex justify-between items-center hover:border-mint-200 dark:hover:border-mint-900 transition-colors group"
						>
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<p className="text-xs text-gray-400">
										{memory.caption || "Untitled"} • {memory.date}
									</p>
								</div>
							</div>
							{!isReadOnly && (
								<button
									...
									className="w-10 h-10 rounded-full bg-ivory dark:bg-gray-700 flex items-center justify-center text-gray-300 hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900 transition-colors border border-gray-100 dark:border-gray-600"
								>
									<Trash2 size={16} />
								</button>
							)}
