...
import { Compass, Home, Plus, Users, Bell, History, Search, X, Settings, HelpCircle, FileText, Info, LogOut, Edit2, Moon, Sun, Globe, ArrowLeft, Link, ChevronRight, MapIcon, Calendar, Heart, BellOff } from 'lucide-react';
import { TEXT, MOCK_USERS, MOCK_TRIPS, MOCK_INVITATIONS } from '@/lib/constants';
import { User, Language, Trip, Invitation, AppNotification, DashboardTabState } from '@/lib/types';
    ...
    <div className="min-h-screen bg-ivory dark:bg-gray-900 pb-32">
      ...
      <div className="bg-mint-400 pt-8 pb-10 px-6 rounded-b-[40px] shadow-lg mb-6 relative z-10 transition-all">
        <div className="flex justify-between items-center text-white">
          ...
          <div onClick={() => setDashboardTab('PROFILE')} className="cursor-pointer">
        ...
        {dashboardTab === 'HOME' && (
          <>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <h3 className="font-bold text-lg">{language === 'ja' ? '新しい旅行を計画する' : 'Plan a New Trip'}</h3>
                    <p className="text-mint-100 text-xs">Create a new itinerary</p>
                  </div>
                </div>
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">{language === 'ja' ? '進行中' : 'Upcoming'}</h2>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[24px] p-4 border border-gray-100 dark:border-gray-700">
              <div className="flex gap-4 items-center">
                <input 
                  ...
                  placeholder={language === 'ja' ? 'タイトルで検索...' : 'Search by title...'}
                />
                <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                  <input type="checkbox" checked={searchOnlySaved} onChange={e => setSearchOnlySaved(e.target.checked)} className="w-4 h-4" />
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <div className="flex gap-2">
                        {trip.isSaved && <span className="text-[10px] text-pink-500 bg-pink-50 dark:bg-pink-900/20 px-2 py-1 rounded-full font-bold">Saved</span>}
                        {trip.status === 'completed' && <span className="text-[10px] bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-300 px-2 py-1 rounded-full font-bold">{t.tabPast}</span>}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      {trip.members.length > 3 && <span className="text-xs text-gray-400">+{trip.members.length - 3}</span>}
                    </div>
                  </div>
                ))}
              {trips
                .filter(t => (searchOnlySaved ? t.isSaved : true))
                .filter(t => t.title.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                <div className="text-center py-16 text-gray-400">
                  {language === 'ja' ? '該当する旅行が見つかりませんでした' : 'No trips found'}
                </div>
              )}
            </div>
          </div>
        ...
        {dashboardTab === 'SAVED' && (
          <div className="space-y-4">
            {trips.filter(t => t.isSaved).length === 0 ? (
              ...
              trips.filter(t => t.isSaved).map(trip => (
                <div 
                  key={trip.id} 
                  onClick={() => router.push(`/trips/${trip.id}`)} 
                  className="bg-white dark:bg-gray-800 rounded-[24px] p-6 border border-gray-100 dark:border-gray-700 cursor-pointer hover:shadow-md transition-shadow"
                >
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-bold text-gray-800 dark:text-white">{trip.title}</h3>
                    <span className="text-[10px] text-pink-500 bg-pink-50 dark:bg-pink-900/20 px-2 py-1 rounded-full font-bold">Saved</span>
                  </div>
                  <p className="text-sm text-gray-400">{trip.dates}</p>
                </div>
              ))
        ...
        {dashboardTab === 'HISTORY' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">{t.history}</h2>
            </div>
            {trips.length === 0 ? (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border ${
                        ...
                        {trip.status === 'completed' ? t.tabPast : (language === 'ja' ? '進行中' : 'Upcoming')}
                      </span>
                      <h3 className="font-bold text-gray-800 dark:text-white line-clamp-1">{trip.title}</h3>
                    </div>
        ...
        {dashboardTab === 'INVITATIONS' && (
          <div className="space-y-4">
            {invitations.length === 0 ? (
              <p className="text-center text-gray-400 mt-10">No pending invitations</p>
            ) : (
              invitations.map(inv => (
                <div key={inv.id} className="bg-white dark:bg-gray-800 rounded-[24px] p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                      className="flex-1 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 py-2 rounded-xl font-bold text-sm"
                    >
                      {t.inviteDecline}
                    </button>
                  </div>
          ...
          <div className="space-y-4">
            {notificationPermission === 'default' && (
              <div className="bg-mint-50 dark:bg-mint-900/20 p-6 rounded-[24px] text-center border border-mint-100 dark:border-mint-900 mb-6">
                ...
                <h3 className="font-bold text-gray-800 dark:text-white mb-1">{language === 'ja' ? '通知を許可' : 'Allow Notifications'}</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">{language === 'ja' ? '旅の更新やメッセージを見逃さないように通知を許可しましょう。' : 'Don\'t miss out on trip updates and messages.'}</p>
                <button className="bg-mint-400 text-white px-6 py-2 rounded-full font-bold text-sm shadow-md hover:bg-mint-500 transition-colors">
            ...
            {notificationPermission === 'denied' && (
              <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-[24px] flex gap-4 items-center border border-red-100 dark:border-red-900 mb-6">
                ...
                <div>
                  <h3 className="font-bold text-gray-800 dark:text-white text-sm">{language === 'ja' ? '通知がブロックされています' : 'Notifications Blocked'}</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{language === 'ja' ? 'ブラウザの設定から通知を許可してください。' : 'Please enable notifications in your browser settings.'}</p>
                </div>
              </div>
            )}
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <div key={n.id} className={`p-4 rounded-2xl flex gap-4 ${n.isRead ? 'bg-gray-50 dark:bg-gray-800' : 'bg-white dark:bg-gray-700 shadow-sm border border-mint-100 dark:border-gray-600'}`}>
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${n.isRead ? 'bg-gray-300' : 'bg-mint-400'}`}></div>
                  <div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-[32px] p-8 text-center border border-gray-100 dark:border-gray-700 relative">
              <div className="absolute top-4 right-4 flex gap-2">
                <button 
                  onClick={() => { setEditNameInput(currentUser.name); setIsEditProfileModalOpen(true); }} 
                  className="p-2 bg-gray-50 dark:bg-gray-700 rounded-full text-gray-400 hover:text-mint-500 hover:bg-mint-50 dark:hover:bg-mint-900/20 transition-colors"
                >
                  <Edit2 size={16} />
                </button>
              </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[32px] p-6 border border-gray-100 dark:border-gray-700 space-y-2">
              ...
              >
                <div className="flex items-center gap-4 text-gray-800 dark:text-white">
                  ...
                  <span className="font-bold">{language === 'ja' ? '設定' : 'Settings'}</span>
                </div>
              ...
              <div className="flex justify-between items-center p-4 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-2xl cursor-pointer transition-colors">
                <div className="flex items-center gap-4 text-gray-800 dark:text-white">
                  ...
                  <span className="font-bold">{language === 'ja' ? 'ヘルプ' : 'Help'}</span>
                </div>
                <ChevronRight size={18} className="text-gray-300" />
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 px-6 py-2 flex justify-between items-center z-40 pb-safe">
        ...
        >
          <History size={24} strokeWidth={dashboardTab === 'HISTORY' ? 2.5 : 2} />
        </button>
        ...
        >
          <Users size={24} strokeWidth={dashboardTab === 'INVITATIONS' ? 2.5 : 2} />
          {invitations.length > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full ring-2 ring-white dark:ring-gray-800"></span>}
        </button>
        <button 
        ...
        >
          <Bell size={24} strokeWidth={dashboardTab === 'NOTIFICATIONS' ? 2.5 : 2} />
          {notifications.some(n => !n.isRead) && <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full ring-2 ring-white dark:ring-gray-800"></span>}
        </button>
      </div>
      ...
      {isActionSheetOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setIsActionSheetOpen(false)}></div>
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-t-[32px] p-8 z-10 animate-slide-up space-y-4 shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-gray-800 dark:text-white text-center mb-6">{language === 'ja' ? 'アクションを選択' : 'Select Action'}</h3>
            
            <button 
              onClick={() => { setIsCreateTripModalOpen(true); setIsActionSheetOpen(false); }} 
            ...
            >
              ...
              <div className="text-left">
                <h4 className="font-bold text-gray-800 dark:text-white text-lg">{t.createTrip}</h4>
                <p className="text-xs text-gray-400">Create a new itinerary from scratch</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                <h4 className="font-bold text-gray-800 dark:text-white text-lg">{t.joinTrip}</h4>
                <p className="text-xs text-gray-400">Enter code or link to join</p>
              </div>
            </button>
      ...
      {isSettingsModalOpen && (
        <div className="fixed inset-0 bg-white dark:bg-gray-900 z-50 flex flex-col animate-fade-in">
          <div className="px-6 py-4 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md sticky top-0 border-b border-gray-100 dark:border-gray-800 flex items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">{language === 'ja' ? '設定' : 'Settings'}</h2>
          </div>
          
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            <section>
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 ml-2">{language === 'ja' ? '一般' : 'General'}</h3>
            ...
            <section>
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 ml-2">{language === 'ja' ? '通知' : 'Notifications'}</h3>
              <div className="bg-gray-50 dark:bg-gray-800 rounded-[24px] overflow-hidden border border-gray-100 dark:border-gray-700">
                <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 cursor-pointer" onClick={() => toggleNotifSetting('enabled')}>
                  <div className="flex items-center gap-3 text-gray-700 dark:text-gray-200">
                    <Bell size={20} />
                    <span className="font-medium">{language === 'ja' ? '通知を許可' : 'Allow Notifications'}</span>
                  </div>
                  <div className={`w-12 h-7 rounded-full p-1 transition-colors ${notifSettings.enabled ? 'bg-mint-400' : 'bg-gray-200 dark:bg-gray-600'}`}>
                    <div className={`w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${notifSettings.enabled ? 'translate-x-5' : ''}`}></div>
                  </div>
                </div>
                {notifSettings.enabled && (
                  <>
                    <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 cursor-pointer pl-10" onClick={() => toggleNotifSetting('messages')}>
                      <span className="text-gray-700 dark:text-gray-200 text-sm">{language === 'ja' ? 'メッセージ通知' : 'Message Notifications'}</span>
                      <div className={`w-10 h-6 rounded-full p-1 transition-colors ${notifSettings.messages ? 'bg-mint-400' : 'bg-gray-200 dark:bg-gray-600'}`}>
                        <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${notifSettings.messages ? 'translate-x-4' : ''}`}></div>
                      </div>
                    </div>
                    <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 cursor-pointer pl-10" onClick={() => toggleNotifSetting('reminders')}>
                      <span className="text-gray-700 dark:text-gray-200 text-sm">{language === 'ja' ? 'リマインダー' : 'Reminders'}</span>
                      <div className={`w-10 h-6 rounded-full p-1 transition-colors ${notifSettings.reminders ? 'bg-mint-400' : 'bg-gray-200 dark:bg-gray-600'}`}>
                        <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${notifSettings.reminders ? 'translate-x-4' : ''}`}></div>
                      </div>
                    </div>
                  </>
                )}
                <div className="flex justify-between items-center p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer" onClick={() => addNotification('Test', 'Notification test')}>
                  <span className="text-mint-500 font-bold text-sm ml-2">{language === 'ja' ? '通知テスト' : 'Test Notification'}</span>
                  <ChevronRight size={18} className="text-gray-300" />
                </div>
              </div>
            </section>
            ...
            <section>
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 ml-2">{language === 'ja' ? 'サポート' : 'Support'}</h3>
              <div className="bg-gray-50 dark:bg-gray-800 rounded-[24px] overflow-hidden border border-gray-100 dark:border-gray-700">
                <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700">
                  <div className="flex items-center gap-3 text-gray-700 dark:text-gray-200">
                    <HelpCircle size={20} />
                    <span className="font-medium">{language === 'ja' ? 'ヘルプ' : 'Help'}</span>
                  </div>
                  <ChevronRight size={18} className="text-gray-300" />
                </div>
                <div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700">
                  <div className="flex items-center gap-3 text-gray-700 dark:text-gray-200">
                    <FileText size={20} />
                    <span className="font-medium">{language === 'ja' ? 'プライバシーポリシー' : 'Privacy Policy'}</span>
                  </div>
                  <ChevronRight size={18} className="text-gray-300" />
                </div>
                <div className="flex justify-between items-center p-4">
                  <div className="flex items-center gap-3 text-gray-700 dark:text-gray-200">
                    <Info size={20} />
                    <span className="font-medium">{language === 'ja' ? 'アプリについて' : 'About'}</span>
                  </div>
                  <ChevronRight size={18} className="text-gray-300" />
                </div>
              </div>
            ...
            <div className="text-center pt-8 pb-10">
              <p className="text-xs text-gray-400 font-medium">TripLink v1.0.0</p>
              <p className="text-[10px] text-gray-300 mt-1">Made with ❤️ for travelers</p>
            </div>
          </div>
        </div>
      )}
      ...
      {isEditProfileModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-6">
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-[32px] p-8 shadow-2xl">
            <h3 className="font-bold text-xl mb-4 text-gray-800 dark:text-white">{language === 'ja' ? 'プロフィール編集' : 'Edit Profile'}</h3>
            <input 
              value={editNameInput} 
            ...
            <div className="space-y-2">
              <button 
                ...
                className="flex-1 py-3 rounded-full text-gray-500"
              >
                {t.cancel}
              </button>
              <button 
                onClick={handleUpdateProfile} 
                className="flex-1 py-3 rounded-full bg-mint-400 text-white font-bold"
              >
                {t.save}
              </button>
            </div>
