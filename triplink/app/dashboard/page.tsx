...
export default function DashboardPage() {
	...
	const handleCardKeyDown = (e: KeyboardEvent, href: string) => {
	...
	return (
		<div className="min-h-screen bg-ivory dark:bg-gray-900 pb-32">
			...
			{/* red Banner Header */}
			<div className="bg-red-400 pt-8 pb-10 px-6 rounded-b-[40px] shadow-lg mb-6 relative z-10 transition-all">
			...
			{dashboardTab === "HOME" && (
				<div className="space-y-6">
					<button
						...
						className="w-full bg-gradient-to-r from-red-400 to-red-500 text-white p-5 rounded-[24px] shadow-lg shadow-red-200 dark:shadow-none flex items-center justify-between group hover:scale-[1.02] transition-transform"
					>
						<div className="flex items-center gap-4">
							...
							<div className="text-left">
								...
								<p className="text-red-100 text-xs">Create a new itinerary</p>
							</div>
						</div>
						...
						{trips.filter((t) => t.status === "upcoming").length === 0 ? (
							<div className="text-center py-20 bg-white dark:bg-gray-800 rounded-[32px] border border-dashed border-gray-300 dark:border-gray-700">
								...
								<button
									type="button"
									onClick={() => setIsCreateTripModalOpen(true)}
									className="bg-red-400 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-red-500"
								>
									{t.planNew}
								</button>
							</div>
							...
							<div className="grid gap-6">
								{trips
									...
									.map((trip) => (
										/* biome-ignore lint/a11y/useSemanticElements: card wrapper contains nested interactive elements */
										<div
											role="button"
											tabIndex={0}
											key={trip.id}
											...
											onKeyDown={(e) =>
												handleCardKeyDown(e, `/trips/${trip.id}`)
											}
										...
										>
											<div className="flex justify-between items-start mb-8">
												<div
													className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-bold ${trip.id === "t1" ? "bg-peach-100 text-peach-500" : "bg-red-100 text-red-500"}`}
												>
					...
					<div className="bg-white dark:bg-gray-800 rounded-[24px] p-4 border border-gray-100 dark:border-gray-700">
						<div className="flex gap-4 items-center">
							<input
								...
								className="flex-1 bg-gray-50 dark:bg-gray-700 dark:text-white p-3 rounded-2xl outline-none focus:ring-2 ring-red-200"
								placeholder={
									language === "ja" ? "タイトルで検索..." : "Search by title..."
								}
							/>
						...
						{trips
							...
							.map((trip) => (
								...
								<div
									...
									key={trip.id}
									onClick={() => router.push(`/trips/${trip.id}`)}
									onKeyDown={(e) => handleCardKeyDown(e, `/trips/${trip.id}`)}
										...
										<span
											className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border ${
												trip.status === "completed"
													...
													: "bg-red-50 text-red-500 border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-900"
											}`}
										>
											{trip.status === "completed"
												? t.tabPast
							...
							>
								...
								<div className="flex gap-2">
									<button
										...
										className="flex-1 bg-red-400 text-white py-2 rounded-xl font-bold text-sm shadow-sm"
									>
										{t.inviteAccept}
									</button>
									<button
				...
				<div className="space-y-4">
					{notifications.length === 0 ? (
						...
						notifications.map((n) => (
							<div
								key={n.id}
								className={`p-4 rounded-2xl flex gap-4 ${n.isRead ? "bg-gray-50 dark:bg-gray-800" : "bg-white dark:bg-gray-700 shadow-sm border border-red-100 dark:border-gray-600"}`}
							>
								<div
									className={`w-2 h-2 rounded-full mt-2 ${n.isRead ? "bg-gray-300" : "bg-red-400"}`}
								></div>
								<div>
			...
			{dashboardTab === "PROFILE" && (
				<div className="space-y-6">
					<div className="bg-white dark:bg-gray-800 rounded-[32px] p-8 text-center border border-gray-100 dark:border-gray-700 relative">
						<div className="absolute top-4 right-4">
							<button
								...
								className="p-2 bg-gray-50 dark:bg-gray-700 rounded-full text-gray-400 hover:text-red-500"
							>
								<Edit2 size={16} />
							</button>
						</div>
						<div className="inline-block p-1 rounded-full border-2 border-red-100 dark:border-red-900 mb-4">
					...
					<div className="bg-white dark:bg-gray-800 rounded-[32px] p-6 border border-gray-100 dark:border-gray-700 space-y-2">
						<button
							type="button"
							className="w-full flex justify-between items-center p-4 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-2xl cursor-pointer transition-colors text-left"
			...
			<div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 px-6 py-2 flex justify-between items-center z-40 pb-safe">
				<button
					...
					className={`p-2 rounded-xl transition-colors flex flex-col items-center gap-1 ${dashboardTab === "HOME" ? "text-red-400" : "text-gray-300"}`}
				>
				...
				<button
					...
					className={`p-2 rounded-xl transition-colors flex flex-col items-center gap-1 ${dashboardTab === "HISTORY" ? "text-red-400" : "text-gray-300"}`}
				>
					<History
						size={24}
				...
				<div className="relative -top-6">
					<button
						...
						className="w-16 h-16 bg-red-400 rounded-full text-white flex items-center justify-center shadow-lg border-4 border-ivory dark:border-gray-900 hover:scale-105 transition-transform hover:shadow-xl hover:bg-red-500"
					>
						<Plus size={32} />
					</button>
				</div>
				...
				<button
					...
					className={`p-2 rounded-xl transition-colors relative flex flex-col items-center gap-1 ${dashboardTab === "INVITATIONS" ? "text-red-400" : "text-gray-300"}`}
				>
					<Users
						size={24}
				...
				<button
					...
					className={`p-2 rounded-xl transition-colors relative flex flex-col items-center gap-1 ${dashboardTab === "NOTIFICATIONS" ? "text-red-400" : "text-gray-300"}`}
				>
					<Bell
						size={24}
				...
				<div className="fixed inset-0 z-50 flex items-end justify-center">
					...
					<div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-t-[32px] p-8 z-10 animate-slide-up space-y-4 shadow-2xl">
						...
						<button
							...
							className="w-full flex items-center gap-4 p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors group border border-transparent hover:border-red-200"
						>
							<div className="w-12 h-12 bg-red-400 rounded-full text-white flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
								<Plus size={24} />
							</div>
							<div className="text-left">
			...
			{isCreateTripModalOpen && (
				<div className="fixed inset-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md z-50 p-8 flex flex-col animate-fade-in">
					...
					<div className="space-y-6 flex-1">
						<div>
							...
							<input
								...
								className="w-full bg-ivory dark:bg-gray-800 dark:text-white p-5 rounded-2xl text-lg font-bold outline-none focus:ring-2 ring-red-200"
					...
					<button
						...
						className="w-full bg-red-400 text-white py-5 rounded-full font-bold text-lg shadow-xl hover:bg-red-500 transition-all"
					>
						{t.create}
					</button>
				</div>
					...
					<div className="flex-1 overflow-y-auto p-6 space-y-8">
							...
							<div className="bg-gray-50 dark:bg-gray-800 rounded-[24px] overflow-hidden border border-gray-100 dark:border-gray-700">
								<div className="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700">
									...
									<button
										...
										className={`w-12 h-7 rounded-full p-1 transition-colors ${isDarkMode ? "bg-red-400" : "bg-gray-200 dark:bg-gray-600"}`}
									>
										<div
						...
						<section>
							...
							<div className="bg-gray-50 dark:bg-gray-800 rounded-[24px] overflow-hidden border border-gray-100 dark:border-gray-700">
								<button
									type="button"
									className="w-full flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 cursor-pointer text-left"
								...
								>
									...
									<div
										className={`w-12 h-7 rounded-full p-1 transition-colors ${notifSettings.enabled ? "bg-red-400" : "bg-gray-200 dark:bg-gray-600"}`}
									>
										<div
								...
								{notifSettings.enabled && (
									<>
										<button
											type="button"
											className="w-full flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 cursor-pointer pl-10 text-left"
											onClick={() => toggleNotifSetting("messages")}
											aria-pressed={notifSettings.messages}
										>
											...
											<div
												className={`w-10 h-6 rounded-full p-1 transition-colors ${notifSettings.messages ? "bg-red-400" : "bg-gray-200 dark:bg-gray-600"}`}
											>
												<div
										...
										<button
											...
											aria-pressed={notifSettings.reminders}
										>
											<span className="text-gray-700 dark:text-gray-200 text-sm">
												{language === "ja" ? "リマインダー" : "Reminders"}
											</span>
											<div
												className={`w-10 h-6 rounded-full p-1 transition-colors ${notifSettings.reminders ? "bg-red-400" : "bg-gray-200 dark:bg-gray-600"}`}
											>
												<div
								...
								<button
									type="button"
									className="w-full flex justify-between items-center p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer text-left"
					...
					<div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-[32px] p-8 shadow-2xl">
						...
						<div className="space-y-2">
							...
							<button
								...
								className="w-full py-3 rounded-full bg-red-400 text-white font-bold"
							>
								{t.save}
							</button>
						</div>
