...
import { Expense, Category, User, Language } from '../types';
...
const COLORS = ['#FB923C', '#99F6E4', '#93C5FD', '#C4B5FD', '#E5E7EB']; 
...
export const Wallet: React.FC<WalletProps> = ({ expenses, members, onAddExpense, onUpdateExpense, onDeleteExpense, isReadOnly = false, language = 'ja' }) => {
  ...
  const data = Object.values(Category).map(cat => {
    return { name: t.categories[cat] || cat, value: expenses.filter(e => e.category === cat).reduce((sum, e) => sum + e.amount, 0) };
  ...
  return (
    <div className="pb-8">
       ...
       {expenses.length > 0 && (
         <div className="bg-white dark:bg-gray-800 rounded-[32px] p-6 mb-8 border border-gray-200 dark:border-gray-700 h-64">
