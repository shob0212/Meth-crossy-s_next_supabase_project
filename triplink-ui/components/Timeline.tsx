...
import { ItineraryEvent, Category, Language } from '../types';
...
export const Timeline: React.FC<TimelineProps> = ({ events, onAddEvent, onUpdateEvent, onDeleteEvent, tripLocation, tripDates, isReadOnly = false, language = 'ja' }) => {
  ...
  const handleAiGenerate = async () => {
    if (!aiPrompt) return;
    setIsLoading(true);
    const suggestions = await generateItinerarySuggestions(tripLocation, activeDateForAi, aiPrompt, language as Language);
    suggestions.forEach((item: any) => onAddEvent({ id: Math.random().toString(36).substr(2, 9), date: activeDateForAi || startDateStr, ...item }));
    setIsLoading(false);
